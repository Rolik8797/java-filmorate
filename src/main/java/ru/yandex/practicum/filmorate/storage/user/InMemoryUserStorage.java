...
public class InMemoryUserStorage implements UserStorage {
    ...
    public User update(User user) {
        if (!users.containsKey(user.getId())) {
            throw new FilmUserNotFoundException(String.format("Пользователя с id=%d нет в базе", user.getId()));
        }
    ...
    public User getUserById(Integer id) {
        if (!users.containsKey(id)) {
            throw new FilmUserNotFoundException(String.format("Пользователь с id=%d не найден", id));
        }
        return users.get(id);
    }
