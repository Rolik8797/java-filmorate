...
class UserControllerTest {
    ...
    @Test
    void shouldReturn201WhenPostCorrectUserData() throws Exception {
        User user = new User(0,
                "correct.email@mail.ru",
                "correctlogin",
                "Correct Name",
                LocalDate.of(2002, 1, 1),
                new ArrayList<>());
        Mockito.when(userController.create(Mockito.any())).thenReturn(ResponseEntity.status(HttpStatus.CREATED).body(user));

        mockMvc.perform(post("/users")
                        .content(objectMapper.writeValueAsString(user))
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated()) // Use status().isCreated() for HTTP 201
